...
export function createShowAddRegexRuleModal(deps: any) {
  const showAddRegexRuleModal = (editRuleId?: string) => {
    ...
    // 获取要编辑的规则(如果有)
    const editRule = editRuleId ? deps.RegexTransformationManager.getRule(editRuleId) : null;

    // 获取所有表格名
    const tableData = deps.getCachedRawData() || deps.getTableData();
        ...
        <div class="acu-edit-dialog acu-validation-modal acu-validation-rule-editor-dialog ${currentThemeClass}">
          ...
          <div class="acu-validation-modal-body">
            <div class="acu-setting-row">
              ...
              <input type="text" id="rule-name" class="acu-panel-input" value="${editRule ? deps.escapeHtml(editRule.name) : ''}" placeholder="例如: 清理多余空格" style="flex:1;" required>
            </div>
            <div class="acu-setting-row">
              <div class="acu-setting-info"><span class="acu-setting-label">规则描述</span></div>
              <textarea id="rule-description" class="acu-panel-input" placeholder="输入规则介绍" style="flex:1; resize: none; overflow-wrap: break-word; overflow-y: hidden; min-height: 34px; height: 34px; line-height: 1.4;">${editRule ? deps.escapeHtml(editRule.description || '') : ''}</textarea>
            </div>
            <div class="acu-setting-row">
              <div class="acu-setting-info"><span class="acu-setting-label">匹配模式 *</span></div>
              <input type="text" id="rule-pattern" class="acu-panel-input" value="${editRule ? deps.escapeHtml(editRule.pattern) : ''}" placeholder="例如: \\s+ 或 /test/g" style="flex:1; font-family: monospace;" required>
            </div>
            <div class="acu-setting-row">
              <div class="acu-setting-info"><span class="acu-setting-label">替换内容</span></div>
              <input type="text" id="rule-replacement" class="acu-panel-input" value="${editRule ? deps.escapeHtml(editRule.replacement || '') : ''}" placeholder="留空表示删除,或使用 $1, $2 等捕获组" style="flex:1; font-family: monospace;">
            </div>
            <div class="acu-setting-row">
    ...
    dialog.find('#acu-regex-rule-confirm').on('click', async function () {
      ...
      const { flags: extractedFlags, patternWithoutFlags } = deps.RegexTransformationEngine._extractFlags(patternStr);
      ...
      if (editRuleId) {
        // 更新现有规则
        deps.RegexTransformationManager.updateRule(editRuleId, ruleData);
      } else {
        // 添加新规则
        const newRule = deps.RegexTransformationManager.addCustomRule(ruleData);
        if (newRule) {
      ...
      deps.refreshRegexRulesList(); // 刷新规则列表而不是重渲染整个界面
    });
