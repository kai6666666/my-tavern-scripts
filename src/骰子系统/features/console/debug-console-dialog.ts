 ...
 */
import { ConsoleCaptureManager } from './console-capture-manager';
import { Store } from '../../shared/storage/store';
import { showActionableErrorToast } from '../../shared/actionable-error-toast';
export function createShowDebugConsoleModal(deps: any) {
      ...
      filteredLogs.forEach(log => {
        const logItem = $(`
          <div class="acu-debug-log-item" data-type="${log.type}">
            ...
            <div class="acu-debug-log-content">${deps.escapeHtml(log.content)}</div>
            ${log.stack ? `<div class="acu-debug-log-stack">${deps.escapeHtml(log.stack)}</div>` : ''}
          </div>
        `);
