...
import { getColumnExamples, getNearestValidNumber, getRelationOptions, isValueInRelationTable } from '../../shared/table-utils';
...
export function createShowSmartFixModal(deps: any) {
  const showSmartFixModal = error => {
    ...
    // ========================================
    if (isTableRule) {
      deps.showTableRuleFixModal(error, rule, ruleType, rawData, snapshot, currentThemeClass);
      return;
    }
    ...
    if (ruleType === 'enum' && rule.config?.values) {
      inputHtml = `<textarea id="smart-fix-value" class="acu-edit-textarea" spellcheck="false"
        style="width:100%;min-height:60px;max-height:200px;resize:none;">${deps.escapeHtml(error.currentValue || '')}</textarea>`;
    ...
    if (ruleType === 'required') {
      ...
      smartSuggestHtml = `
        <div class="acu-smart-fix-suggest">
          <div class="acu-smart-fix-suggest-label">
            <i class="fa-solid fa-font"></i> 格式要求: <code>${deps.escapeHtml(rule.config.pattern)}</code>
          </div>
          ${
            suggestedValue
              ? `
            <div style="margin-top:8px;">
              ...
              <span class="acu-smart-fix-quick-btn" data-value="${deps.escapeHtml(suggestedValue)}">
                <i class="fa-solid fa-magic"></i> ${deps.escapeHtml(suggestedValue)}
              </span>
            </div>
          `
              : ''
      ...
      if (options.length > 0) {
        optionsHtml = `
          <div class="acu-smart-fix-suggest-label">
            <i class="fa-solid fa-link"></i> 关联表 "${deps.escapeHtml(rule.config.refTable)}" 可用值 (${options.length}项):
          </div>
        ...
        if (hasMultipleColumns) {
          ...
          reverseWriteHtml = `
            <div class="acu-smart-fix-reverse-write" style="margin-top:15px;padding-top:15px;border-top:1px solid var(--acu-border);">
              ...
              <div style="margin-top:8px;">
                <button class="acu-smart-fix-quick-btn" id="smart-fix-reverse-write-btn" data-column="${deps.escapeHtml(refColumns[0])}" style="width:100%;">
                  <i class="fa-solid fa-plus"></i> 将 "${deps.escapeHtml(currentInvalidValue.length > 30 ? currentInvalidValue.substring(0, 30) + '...' : currentInvalidValue)}" 写入到 "${deps.escapeHtml(rule.config.refTable)}.${deps.escapeHtml(refColumns[0])}"
                </button>
              </div>
            </div>
          `;
          ...
          <div style="margin-bottom:8px;">
            ${issues
              ...
              <div style="font-size:11px;color:var(--acu-text-sub);padding:4px 8px;background:var(--acu-card-bg);border-radius:4px;margin-bottom:4px;">
                <span style="color:var(--acu-hl-manual);">❌</span> ${deps.escapeHtml(issue.pair)} - ${deps.escapeHtml(issue.error)}
              </div>
            `,
              )
              .join('')}
            ${fixedPairs
              ...
              .map(
                p => `
              <div style="font-size:11px;color:var(--acu-text-sub);padding:4px 8px;background:var(--acu-card-bg);border-radius:4px;margin-bottom:4px;">
                <span style="color:var(--acu-hl-manual);">⚠️</span> ${deps.escapeHtml(p.key)}:${deps.escapeHtml(p.originalVal)} → ${deps.escapeHtml(p.val)}
              </div>
            `,
              )
              .join('')}
        ...
        <div class="acu-smart-fix-suggest">
          ...
          ${
            hasIssues
              ? `
            <div style="margin-top:8px;">
              <span class="acu-smart-fix-quick-btn" data-value="${deps.escapeHtml(fixedValue)}">
            ...
            <div style="margin-top:8px;padding:8px;background:var(--acu-card-bg);border-radius:4px;font-size:11px;color:var(--acu-text-sub);">
              ...
              <code style="color:var(--acu-success-text);">${deps.escapeHtml(fixedValue)}</code>
            </div>
          `
              : `
    ...
    const dialog = $(`
      <div class="acu-edit-overlay acu-validation-modal-overlay">
        <div class="acu-edit-dialog acu-validation-modal ${currentThemeClass}" style="max-width:450px;">
          <div class="acu-edit-title">智能修改: ${deps.escapeHtml(error.tableName || '')} - ${deps.escapeHtml(error.columnName || '')}</div>
          <div class="acu-settings-content" style="flex:1; overflow-y:auto; padding:15px;">
            ...
            <div class="acu-smart-fix-rule-info">
              <div class="acu-smart-fix-rule-header">
                ...
                <span>${deps.escapeHtml(error.ruleName || rule.name || '')}</span>
              </div>
              <div class="acu-smart-fix-rule-desc">${deps.escapeHtml(error.errorMessage || rule.errorMessage || typeInfo.desc || '')}</div>
            </div>
            ...
            ${
              ...
              <div class="acu-diff-section acu-diff-old-section">
                ...
                <div class="acu-diff-readonly">${deps.escapeHtml(snapshotValue)}</div>
              </div>
            ...
            <div class="acu-diff-section acu-diff-new-section">
              ${
                ...
                <i class="fa-solid fa-tag" style="margin-right:4px;"></i>${deps.escapeHtml(error.rowTitle)}
              </div>`
                  : ''
              }
              <div class="acu-diff-label">
    ...
    dialog.on('click', '#smart-fix-reverse-write-btn', async function () {
      ...
      if (ruleType === 'relation' && rule.config?.refColumn) {
        ...
        deps.errorTableTemplateIssue('无法确定目标列');
        return;
      }

      if (!targetColumn || !rule.config?.refTable) {
        deps.errorTableTemplateIssue('无法确定目标表或列');
        return;
      }
      ...
      try {
        ...
        if (!refSheet || !refSheet.content) {
          deps.errorTableTemplateIssue(`找不到关联表 "${rule.config.refTable}"`);
          return;
        }
        ...
        if (targetColIdx === -1) {
          deps.errorTableTemplateIssue(`关联表中不存在列 "${targetColumn}"`);
          return;
        }
        ...
        await deps.appendRowInstantly(refSheetId || rule.config.refTable, newRow);
          ...
          if (rawData[sheetId]?.name === error.tableName) {
            ...
            if (error.rowIndex >= 0 && error.columnName) {
              ...
              if (colIdx >= 0 && sheet.content && sheet.content[rowIdx]) {
                ...
                await deps.saveRowInstantly(sheetId, error.rowIndex, nextRow);
                updated = true;
                break;
              }
            }
