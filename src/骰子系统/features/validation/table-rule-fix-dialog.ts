    ...
    if (ruleType === 'tableReadonly') {
        ...
        <div class="acu-smart-fix-table-summary">
          ...
          ${
            ...
            <div class="acu-smart-fix-change-list">
              ${changeDetails
                .slice(0, 10)
                .map(d => `<div class="acu-smart-fix-change-item">${deps.escapeHtml(d)}</div>`)
                .join('')}
      ...
      contentHtml = `
        <div class="acu-smart-fix-rule-info">
          ...
          <div class="acu-smart-fix-rule-desc">${deps.escapeHtml(error.errorMessage || rule.errorMessage || '')}</div>
        </div>
        <div class="acu-smart-fix-table-summary">
          <div class="acu-smart-fix-stat">
      ...
      if (isTooMany && excessRows.length > 0) {
        actionBtns = `
          ...
          <button class="acu-dialog-btn acu-btn-confirm" id="smart-fix-delete-rows" data-start="${maxRows}" data-table="${deps.escapeHtml(error.tableName)}">
      ...
      if (targetSheet && targetSheet.content && targetSheet.content.length > 1) {
        ...
        const colIndex = deps.ValidationEngine.findColumnIndex(headers, rule.targetColumn);
        ...
        <div class="acu-smart-fix-table-summary">
          ...
          ${
            ...
            <div class="acu-smart-fix-change-list" style="max-height:200px;overflow-y:auto;margin-top:8px;">
              ${issues
                .slice(0, 20)
                .map(issue => `<div class="acu-smart-fix-change-item">${deps.escapeHtml(issue)}</div>`)
                .join('')}
              ...
              <div class="acu-smart-fix-change-list" style="max-height:200px;overflow-y:auto;margin-top:8px;">
                ${fixSuggestions
                  ...
                  <div class="acu-smart-fix-change-item">
                    ...
                    <span style="color:var(--acu-hl-manual);">${deps.escapeHtml(fix.currentValue)}</span>
                    <span style="color:var(--acu-text-sub);"> → </span>
                    <span style="color:var(--acu-hl-diff);">${deps.escapeHtml(fix.fixedValue)}</span>
                    <span style="color:var(--acu-text-sub);font-size:11px;"> (${deps.escapeHtml(fix.reason)})</span>
                  </div>
                `,
                  )
                  .join('')}
      ...
      if (fixSuggestions.length > 0) {
        ...
        if (pairedTable) {
          ...
          actionBtns = `
            <button class="acu-dialog-btn" id="smart-fix-cancel"><i class="fa-solid fa-times"></i> 取消</button>
            <button class="acu-dialog-btn acu-btn-confirm" id="smart-fix-fix-sequence" data-table="${deps.escapeHtml(error.tableName)}" data-column="${deps.escapeHtml(rule.targetColumn)}" data-prefix="${deps.escapeHtml(prefix)}" data-start="${startFrom}">
    ...
    const dialog = $(`
      <div class="acu-edit-overlay acu-validation-modal-overlay">
        <div class="acu-edit-dialog acu-validation-modal ${currentThemeClass}" style="max-width:450px;">
          <div class="acu-edit-title">智能修改: ${deps.escapeHtml(error.tableName || '')} (表级规则)</div>
    ...
    dialog.on('click', '#smart-fix-fix-sequence', async function () {
      ...
      try {
        ...
        if (!targetSheet || !targetSheet.content || targetSheet.content.length < 2) {
          deps.warnTableTemplateIssue('未找到目标表');
        ...
        if (colIndex < 0) {
          deps.warnTableTemplateIssue('未找到目标列');
          $btn.prop('disabled', false).html('<i class="fa-solid fa-magic"></i> 自动修复');
          return;
        }
        ...
        if (pairedTableName) {
          ...
          if (!pairedSheet || !pairedSheet.content || pairedSheet.content.length < 1) {
            deps.warnTableTemplateIssue(`未找到配对表: ${pairedTableName}`);
            $btn.prop('disabled', false).html('<i class="fa-solid fa-magic"></i> 自动修复');
            return;
          }
          ...
          if (pairedColIndex < 0) {
            deps.warnTableTemplateIssue(`配对表中未找到目标列: ${columnName}`);
            $btn.prop('disabled', false).html('<i class="fa-solid fa-magic"></i> 自动修复');
            return;
          }
          ...
          await deps.saveDataOnly(rawData, [targetSheetId, pairedSheetId].filter(Boolean));
          closeDialog();
          ...
          await deps.saveDataOnly(rawData, targetSheetId ? [targetSheetId] : undefined);
          closeDialog();
          deps.renderInterface();
        }
      } catch (e) {
    ...
    dialog.on('click', '#smart-fix-delete-rows', async function () {
      ...
      try {
        ...
        for (let rowIndex = targetRowCount - 1; rowIndex >= startRow; rowIndex--) {
          await deps.deleteRowInstantly(targetSheetId, rowIndex);
        }
        closeDialog();
        deps.renderInterface();
      } catch (e) {
