...
export function createShowAddValidationRuleModal(deps: any) {
  const showAddValidationRuleModal = (parentDialog: JQuery, editRuleId?: string) => {
    ...
    const existingRule = isEditMode ? deps.ValidationRuleManager.getRule(editRuleId) : null;
          ...
          <div class="acu-validation-modal-body">
            ...
            <div class="acu-setting-row">
              ...
              <select id="rule-table" class="acu-setting-select">
                ...
                ${tableNames.map(name => `<option value="${deps.escapeHtml(name)}">${deps.escapeHtml(name)}</option>`).join('')}
              </select>
            </div>
    ...
    dialog.find('#rule-table').on('change', function () {
      ...
      if ($pairedTableSelect.length > 0) {
        ...
        tableNames
          ...
          .forEach(name => {
            $pairedTableSelect.append(`<option value="${deps.escapeHtml(name)}">${deps.escapeHtml(name)}</option>`);
          });
        // 如果之前选择的值仍然有效，恢复它
    ...
    deps.setupOverlayClose(dialog, 'acu-validation-modal-overlay', () => {
      closeDialog();
    });
    ...
    dialog.find('#dlg-rule-save').on('click', function () {
      ...
      if (isEditMode) {
        // 编辑模式：更新规则
        success = deps.ValidationRuleManager.updateCustomRule(editRuleId, ruleData);
      } else {
        // 新建模式：添加规则
        success = deps.ValidationRuleManager.addCustomRule(ruleData);
      }
      ...
      if (success) {
        ...
        if (isEditMode) {
          ...
          // 新建模式：追加新规则项
          const ruleHtml = `
          <div class="acu-validation-rule-item" data-rule-id="${deps.escapeHtml(finalRuleId)}">
            <div class="acu-rule-type-icon" title="${deps.escapeHtml(typeInfo?.name || ruleType)}${isTableRule ? ' (表级)' : ''}">
              <i class="fa-solid ${typeInfo?.icon || 'fa-question'}"></i>
            </div>
            <div class="acu-rule-info">
              <div class="acu-rule-name">${deps.escapeHtml(name)}</div>
              <div class="acu-rule-target">${deps.escapeHtml(targetTable)}${isTableRule && ruleType !== 'sequence' ? ' (整表)' : '.' + deps.escapeHtml(targetColumn)}</div>
            </div>
            <div class="acu-rule-intercept" data-rule-id="${deps.escapeHtml(finalRuleId)}" title="点击启用拦截提示（违反时标注）"><i class="fa-solid fa-shield-halved"></i></div>
            <button type="button" class="acu-rule-action acu-rule-edit" data-rule-id="${deps.escapeHtml(finalRuleId)}" title="编辑此规则" aria-label="编辑此规则"><i class="fa-solid fa-pen"></i></button>
            <div class="acu-rule-toggle active" title="点击切换启用/禁用">
              <i class="fa-solid fa-toggle-on"></i>
            </div>
            <button type="button" class="acu-rule-action acu-rule-delete" data-rule-id="${deps.escapeHtml(finalRuleId)}" title="删除此规则" aria-label="删除此规则"><i class="fa-solid fa-trash"></i></button>
          </div>
        `;
  ...
  return showAddValidationRuleModal;
}
