...
export function createValidationEngine(deps: any) {
  const ValidationEngine = {
      ...
      for (const rule of rules) {
        ...
        for (const sheetId in snapshot) {
          if (
            ...
            (deps.isNpcTableName(snapshot[sheetId]?.name) && deps.isNpcTableName(rule.targetTable))
          ) {
        ...
        for (const sheetId in newData) {
          if (
            ...
            (deps.isNpcTableName(newData[sheetId]?.name) && deps.isNpcTableName(rule.targetTable))
          ) {
    ...
    validateRow(row, headers, tableName, rowIndex, rules, rawData) {
      ...
      for (const rule of rules) {
        if (rule.targetTable !== tableName && !(deps.isNpcTableName(rule.targetTable) && deps.isNpcTableName(tableName)))
          continue;
