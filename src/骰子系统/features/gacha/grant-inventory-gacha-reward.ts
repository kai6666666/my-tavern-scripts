// @ts-nocheck
/**
 * grant-inventory-gacha-reward.ts
...
export function createGrantInventoryGachaReward(deps: any) {
  const grantInventoryGachaReward = (
    ...
    if (existing) {
      ...
      deps.setInventoryRowBasicFields(row, parsed.colMap, item, nextQuantity);
    ...
    deps.setInventoryRowBasicFields(newRow, parsed.colMap, item, Math.max(1, quantity));
    deps.applyGachaCustomFieldsToRow(newRow, headerRow, item, { target: 'inventory', targetColumns: item.targetColumns });
    ...
    deps.setInventoryMetadataForItem(
      rawData,
      { tableKey: parsed.tableKey, tableName: parsed.tableName, name: item.name },
      deps.buildGachaInventoryMetaRecord(rawData, {
  ...
  return grantInventoryGachaReward;
}
