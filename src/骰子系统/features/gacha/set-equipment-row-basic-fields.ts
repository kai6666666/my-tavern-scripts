// @ts-nocheck
/**
 * set-equipment-row-basic-fields.ts
 * Feature-Sliced 模块（工厂版，DI 注入依赖）。
 */
import type { GachaItemDefinition } from '../../entities/gacha-items';
export function createSetEquipmentRowBasicFields(deps: any) {
  const setEquipmentRowBasicFields = (
    ...
    if (colMap.type >= 0) row[colMap.type] = deps.resolveEquipmentTableTypeForGachaItem(item, headers, colMap, sheet);
  ...
  return setEquipmentRowBasicFields;
}
